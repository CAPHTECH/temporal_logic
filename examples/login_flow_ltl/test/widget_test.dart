...
void main() {
  group('Login Flow LTL Example (External Recording)', () {
    ...
    testWidgets(
        'Successful login flow satisfies LTL formula (EXPECTED TO FAIL due to flicker bug)',
        (tester) async {
      // --- Setup ---
      final recorder =
          tl_flutter.TraceRecorder<AppSnap>(interval: Duration.zero);
      ...
      container.listen<AppState>(appStateProvider, (previous, next) {
        ...
        recorder.record(AppSnap.fromAppState(next));
          ...
          fireImmediately:
              false); // Don't fire immediately, already recorded initial
      ...
      final loading =
          tl_flutter.state<AppSnap>((s) => s.isLoading, name: 'loading');
      final home =
          tl_flutter.state<AppSnap>((s) => s.isOnHomeScreen, name: 'home');
      final error = tl_flutter.state<AppSnap>((s) => s.hasError, name: 'error');
      final loginClicked = tl_flutter.event<AppSnap>((s) => s.loginClicked,
          name: 'loginClicked');

      // LTL Formula φ: Originally defined for a *correct* successful login
      final formula = tl_flutter.always(loginClicked.implies(tl_flutter
          .next(loading)
          .and(tl_flutter.eventually(home))
          .and(tl_flutter.always(error.not()))));
      ...
      expect(
        trace,
        isNot(tl_flutter.satisfiesLtl(formula)),
        reason:
            'LTL formula for successful login should FAIL due to the injected flicker bug violating G(!error).',
      );
    });
    ...
    testWidgets('Failed login flow violates LTL formula part', (tester) async {
      ...
      final formula = tl_flutter.always(loginClicked.implies(tl_flutter
              .next(loading)
              .and(tl_flutter.eventually(home)) // This part will fail
              .and(tl_flutter.always(error.not())) // This part will also fail
          ));

      // Failure formula (same as before)
      final failureFormula =
          tl_flutter.always(loginClicked.implies(tl_flutter.eventually(error)));

      final trace = recorder.trace;

      expect(trace, isNot(tl_flutter.satisfiesLtl(formula)),
          reason:
              'LTL formula for successful login should NOT hold for failure (external recording).');

      expect(trace, tl_flutter.satisfiesLtl(failureFormula),
          reason:
              'LTL formula for failed login should hold (external recording).');
    });

    testWidgets('Successful login should NOT flicker through error state',
        (tester) async {
      ...
      await tester.tap(find.byKey(const Key('login')));
      await tester
          .pumpAndSettle(); // Let login process (including flicker) complete
      ...
      final originalSuccessFormula = tl_flutter.always(loginClicked.implies(
          tl_flutter
              .next(loading)
              .and(tl_flutter.eventually(home))
              .and(tl_flutter.always(error.not()))));
      ...
      final noErrorAfterLoading =
          tl_flutter.always(loading.implies(tl_flutter.always(error.not())));
      // Alternative Formula 2b: "Globally, if login is clicked, the next state is loading, and it's NEVER error afterwards"
      // final noErrorAfterLogin = tl_flutter.always(loginClicked.implies(tl_flutter.next(loading.and(tl_flutter.always(error.not())))));
      ...
      expect(trace, isNot(tl_flutter.satisfiesLtl(originalSuccessFormula)),
          reason:
              'Original success formula should FAIL due to G(!error) violation by flicker.');

      // 2. The specific anti-flicker formula MUST fail because of the bug.
      expect(trace, isNot(tl_flutter.satisfiesLtl(noErrorAfterLoading)),
    ...
    testWidgets('Standard test passes for successful login (misses flicker)',
        (tester) async {
      // --- Setup ---
      ...
      expect(find.byType(LoadingScreen), findsNothing,
          reason:
              'Loading screen should not be present after successful login settles.');
      ...
      expect(find.byType(ErrorScreen), findsNothing,
          reason:
              'Error screen should not be present after successful login settles.');

      // Verify that the home screen is present.
      expect(find.byType(HomeScreen), findsOneWidget,
          reason:
              'Home screen should be present after successful login settles.');
