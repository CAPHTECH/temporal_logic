...
import 'package:temporal_logic_flutter/temporal_logic_flutter_test.dart' as tl;
...
void main() {
  ...
  test('Counter increments with Riverpod respects LTL properties', () async {
    ...
    container.listen<int>(
      ...
      fireImmediately:
          true, // Records the initial state (0) after initialization
    );
    ...
    final propIncrement = tl
        .always(
          tl
              .state<int>((s) => s == 0)
              .implies(tl.next(tl.state<int>((s) => s == 1))),
        ) // always(0 -> next(1))
        .and(
          // Use .and() extension
          tl.always(
            tl
                .state<int>((s) => s == 1)
                .implies(tl.next(tl.state<int>((s) => s == 2))),
          ), // always(1 -> next(2))
        );
    ...
    expect(
      trace,
      tl.satisfiesLtl(propEventually2),
      reason: 'Expected counter to eventually be 2',
    );
    expect(
      trace,
      tl.satisfiesLtl(propAlwaysNonNegative),
      reason: 'Expected counter to always be non-negative',
    );
    expect(
      trace,
      tl.satisfiesLtl(propIncrement),
      reason: 'Expected counter increments correctly (0->1, 1->2)',
    );
    expect(
      trace,
      tl.satisfiesLtl(propNever3),
      reason: 'Expected counter to never reach 3 in this trace',
    );
  });
}
