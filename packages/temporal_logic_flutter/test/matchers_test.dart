...
void main() {
  group('satisfiesLtl Matcher', () {
    ...
    test('matches P.and(eventually(Q)) scenario', () {
      ...
      final q = state<TestState>((s) => !s.value,
          name: 'Q (value=false)'); // Use not operator for clarity
      ...
      final trace = tlCore.Trace<TestState>([
        tlCore.TraceEvent(
            value: TestState(true), timestamp: Duration.zero), // P holds
        tlCore.TraceEvent(
            value: TestState(false),
            timestamp: const Duration(milliseconds: 100)), // Q holds
      ]);
      ...
      expect(trace, satisfiesLtl(formulaCombined),
          reason: 'P && F(Q) should hold for trace [P, Q]');

      // For sanity check, also test eventually(Q) alone
      final formulaEventually = tlCore.eventually(q);
      expect(trace, satisfiesLtl(formulaEventually),
          reason: 'F(Q) should hold for trace [P, Q]');
    });
